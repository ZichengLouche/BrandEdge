define(['BrandEdge/app', 'lib/underscore/underscore', 'lib/underscore/underscore.string'], function(app, underscore, underscore_str){
	app.controller('DevelopersChartDetailsController', function($scope, $rootScope, $controller, $stateParams, $loading, GlobalizationService, MessageFactory, DeveloperChartFactory, BroadcastConstants, DataUtility, column, filter, $q, BHDataAdapterService, TrendFactory) {
		_.str = underscore_str;

        $scope.$emit('developer-tab', $stateParams);
        ...
        this.load = function(countryObject, dateObject) {
            ...
            if (dateObject.current || (dateObject.startDate.year == dateObject.endDate.year && dateObject.startDate.month == dateObject.endDate.month)) {
                DeveloperChartFactory.getData((countryObject)?countryObject.country:null, (dateObject)?dateObject:null, column, filter).then((function(data) {
		        ...
		        var tp = [countryObject.country, 'DEVELOPERS', columns[column], dateObject.startDate.year + '-' + dateObject.startDate.month + '-' + '1', dateObject.endDate.year + '-' + dateObject.endDate.month + '-' + '1', ''];
		        promises.push(BHDataAdapterService.getTrendData(tp));
		        promises.push(DeveloperChartFactory.getData((countryObject)?countryObject.country:null, (dateObject)?dateObject:null, column, filter));
				$q.all(promises).then(
						(function(result)
						{
							$scope.data = [];
							...
							var lineData = TrendFactory._refactorData(result[0], 'DEVELOPERS', column, filter);
							var res = [];
