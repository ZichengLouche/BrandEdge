define(['BrandEdge/app'], function(app) {
	app.controller('ActionController', function($scope, $rootScope, $state, $loading, $q, BroadcastConstants, ViewConstants, ChartService, AlertService, BHDataAdapterService, DataUtility, Utility, TrendFactory) {
		...
		$scope.loadData = function(params) {
			...
			if ((params == null) || (params.country == null)) {
				...
				if (params.date.current || (params.date.startDate.year == params.date.endDate.year && params.date.startDate.month == params.date.endDate.month)) {
					...
					var promises = [];
					var tp = [params.country.country, 'CLIENTS & PROSPECTS', 2, params.date.startDate.year + '-' + params.date.startDate.month + '-' + '1', params.date.endDate.year + '-' + params.date.endDate.month + '-' + '1', ''];
					...
					$q.all(promises).then(
						function(results) {
							...
							var data = {
				    				"marketshare": results[0]["2"],
//				    				"preference": results[0]["2"],
				    				"preference": results[0]["3"],
				    				"engagement": results[0]["4"],
				    				"proposal": results[0]["5"]
								};
							var metric = ['marketshare', 'preference', 'engagement', 'proposal'];
							var linedata = {};
							var chartdata = {};
